...
- Só processa tasks com status = CLICKUP_STATUS_TODO (default:  em rascunho)
...
- Comenta na task e move status para CLICKUP_STATUS_DONE (default: finalizada) se sucesso
"""
...
# Nomes de status padrão (serão validados dinamicamente)
...
def main():
    ...
    try:
        ...
        # Descobre os nomes exatos para evitar erro 400
        status_todo_real = find_real_status_name(CLICKUP_STATUS_TODO, all_statuses) or CLICKUP_STATUS_TODO
        
        # Prioridade para o status DONE: 'concluída' -> 'finalizada' -> qualquer um do tipo 'done' ou 'closed'
        ...
        print(f"   Status TODO será: '{status_todo_real}'")
        print(f"   Status DONE será: '{status_done_real}'")
                ...
                if candidatos:
                    for t in candidatos:
                        ...
                        try:
                            ...
                            if not tipo or not chamados:
                                continue

                            print(f"➡️  Ação: {tipo} | Chamados: {', '.join(chamados)}")
                            erros = 0
                            ...
                            try:
                                ...
                                # Se falhar, tenta o status 'finalizada' como último recurso
                                try:
                                    print("⚙️  Tentando fallback para status 'finalizada'...")
                                    update_status(task_id, "finalizada")
                                    print("✅ Task movida para 'finalizada' (fallback).")
                                except Exception as e2:
                                    print(f"❌ Falha total ao atualizar status: {e2}")
                        except Exception as e:
